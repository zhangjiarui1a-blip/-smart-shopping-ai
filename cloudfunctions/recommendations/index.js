...
function collectProfile(body, query) {
  ...
  return {
    budget: directBudget || Number(criteria.budget) || queryBudget(fullText),
    category: String(criteria.category || '').trim(),
    scenario: directScenario || scenarioFrom(fullText) || '\u672a\u6307\u5b9a',
    preference: directPreference || preferenceFrom(fullText) || '\u7efc\u5408\u6027\u4ef7\u6bd4'
  };
}
...
function buildPrompt(query, profile, candidates) {
  return [
    '\u4f60\u662f\u8d2d\u7269\u9009\u62e9\u52a9\u624b\u3002',
    `\u9700\u6c42\uff1a${query}\uff1b\u9884\u7b97${profile.budget || '\u672a\u6307\u5b9a'}\uff1b\u7c7b\u522b${profile.category || '\u672a\u5206\u7c7b'}\uff1b\u573a\u666f${profile.scenario}\uff1b\u504f\u597d${profile.preference}\u3002`,
...
async function handleRequest(event) {
  ...
  try {
    ...
    console.log('[INPUT]', JSON.stringify({ requestId: context.requestId, query, profile, candidateCount: candidates.length }));
    if (!candidates.length) return returnWithLog(400, { error: 'candidates are required to generate recommendations' }, context);
    if (profile.category && candidates.some(candidate => String(candidate.category || '') !== profile.category)) {
      return returnWithLog(400, { error: 'candidate category validation failed' }, context);
    }
